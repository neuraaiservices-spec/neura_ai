      ...
      <section id="about" className="py-24 lg:py-32 bg-white relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-10 relative z-10">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{once:true,amount:0.3}} variants={stagger} className="text-center mb-20">
            ...
            <motion.h2 variants={fadeUp} className="font-display text-3xl lg:text-5xl text-primary leading-tight mt-2 mb-5">
              We develop affordable<br/>medical solutions.<br/><span className="italic" style={{background:'linear-gradient(135deg,#1d1d1f 0%,#00c9a7 100%)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text'}}>Built for everyone.</span>
            </motion.h2>
            <motion.p variants={fadeUp} className="text-text-mid text-lg max-w-2xl mx-auto leading-relaxed font-light">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center mb-20">
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{once:true,amount:0.3}} variants={stagger} className="flex flex-col gap-6">
              <motion.div variants={fadeUp} className="border-l-2 border-teal pl-5">
                <p className="text-primary/80 text-base leading-relaxed italic font-light">
                  "We focus on making healthcare affordable — through medical devices, software, and education."
                </p>
              </motion.div>
              <motion.p variants={fadeUp} className="text-text-mid text-sm leading-relaxed">
                We develop medical solutions — hardware and software — for conditions like cardiac disease, neurological disorders, kidney health, and respiratory conditions. We also run <strong className="text-primary font-semibold">NeuraEDU</strong>, an education division that gives engineering students practical experience in medical R&D.
              </motion.p>
              <motion.div variants={fadeUp} className="flex flex-wrap gap-2">
                {['Medical Device Development','Healthcare Software','Affordable Diagnostics','NeuraEDU'].map(t => (
                  <span key={t} className="bg-cream border border-black/5 text-text-mid text-xs font-medium px-3 py-1.5 rounded-full">{t}</span>
                ))}
              </motion.div>
            </motion.div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <motion.div initial="hidden" whileInView="visible" viewport={{once:true,amount:0.3}} variants={stagger} className="flex flex-col gap-5">
              ...
              <motion.h3 variants={fadeUp} className="font-display text-3xl lg:text-4xl text-primary">
              ...
              {[
                ...
                <motion.div key={item.n} variants={fadeUp} className="flex gap-5 items-start group">
                  <span className="font-display text-xl text-teal/40 min-w-[32px] pt-0.5 group-hover:text-teal transition-colors">{item.n}</span>
                  <div>
                    <h4 className="font-semibold text-primary text-sm mb-1">{item.t}</h4>
                    <p className="text-text-mid text-sm leading-relaxed">{item.d}</p>
                  </div>
                </motion.div>
              ))}
            </motion.div>
            <motion.div initial={{opacity:0,x:40}} whileInView={{opacity:1,x:0}} transition={{duration:0.8}} viewport={{once:true}}>
              <div className="relative">
                <img src={vision} alt="Our Vision" className="w-full rounded-3xl object-cover shadow-apple-lg" />
                <div className="absolute inset-0 rounded-3xl ring-1 ring-black/5" />
              </div>
            </motion.div>
          </div>
