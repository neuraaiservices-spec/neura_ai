    ...
    <section id="partners" className="py-20 lg:py-28 bg-white">
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
          ...
          {mouPartners.map((p,i) => (
            <motion.div key={p.name} initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.5,delay:i*0.1}}
              ...
              <div style={{height:180,overflow:'hidden',position:'relative'}}>
                ...
                <div style={{position:'absolute',inset:0,background:'linear-gradient(to top,rgba(29,29,31,0.85) 0%,rgba(29,29,31,0.2) 60%)'}}/>
              ...
              <div style={{padding:'16px 20px 20px',background:'white'}}>
                <h3 style={{fontFamily:"Inter,-apple-system,sans-serif",fontSize:17,color:'#1d1d1f',marginBottom:4,lineHeight:1.3}}>{p.name}</h3>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
          {eventColleges.map((c,i) => (
            <motion.div key={c.name} initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.4,delay:i*0.07}}
              ...
              <p style={{color:'#1d1d1f',fontSize:11,fontWeight:600,lineHeight:1.35,marginBottom:2,fontFamily:"Inter,-apple-system,sans-serif"}}>{c.name}</p>
        ...
        <motion.div initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.6}}
          style={{background:'#1d1d1f',borderRadius:24,padding:'40px 48px',textAlign:'center'}}>
          ...
          <h3 style={{fontFamily:"Inter,-apple-system,sans-serif",fontWeight:800,letterSpacing:'-0.02em',color:'white',fontSize:'clamp(22px,2.5vw,32px)',marginBottom:12}}>Partner with NeuraEDU.</h3>
