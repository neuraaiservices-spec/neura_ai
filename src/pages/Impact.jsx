      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 relative z-10">
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{once:true,amount:0.3}} variants={stagger} className="mb-14">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 mt-2">
            <motion.h2 variants={fadeUp} className="font-display text-3xl lg:text-5xl text-primary max-w-xl leading-tight">
            ...
            <motion.p variants={fadeUp} className="text-text-mid text-sm lg:text-base max-w-sm leading-relaxed">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-16">
          {news.map((item, i) => (
            <motion.div key={i}
              ...
              className={`rounded-3xl overflow-hidden border transition-all duration-300 hover:-translate-y-1 shadow-apple ${item.highlight ? 'border-teal/30 hover:border-teal/60' : 'border-black/8 hover:border-blue/40'}`}
            ...
            >
              ...
              {!item.img && (
                <div className="h-24 relative overflow-hidden" style={{background:'linear-gradient(135deg,rgba(0,102,204,0.1),rgba(0,201,167,0.06))'}}>
                  <div className="absolute top-4 left-6 flex items-center gap-2">
                    <span className="text-2xl">🚀</span>
                    <span style={{color:'#0066cc',fontSize:12,fontWeight:700,fontFamily:"Inter, -apple-system, sans-serif",letterSpacing:'0.05em'}}>VentureConnect 2026</span>
                  </div>
                </div>
              )}
              ...
              <div className="p-6">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-text-mid text-xs">{item.date}</span>
                </div>

                <h3 className="font-display text-lg text-primary mb-3 leading-snug">{item.title}</h3>
                <p className="text-text-mid text-sm leading-relaxed mb-4">{item.desc}</p>
                ...
                <a href={item.linkedin} target="_blank" rel="noreferrer"
                  style={{
                    ...
                    color:'#0066cc', padding:'8px 16px', borderRadius:20,
                    fontSize:12, fontWeight:600,
                    fontFamily:"Inter, -apple-system, sans-serif",
                    textDecoration:'none', transition:'all 0.2s',
                  }}
                  onMouseEnter={e => e.currentTarget.style.background='rgba(0,102,204,0.16)'}
                  onMouseLeave={e => e.currentTarget.style.background='rgba(0,102,204,0.08)'}
                >
        ...
        <motion.div initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:0.3}}
          ...
          <p className="text-text-mid text-sm mb-4" style={{fontFamily:"Inter, -apple-system, sans-serif"}}>
